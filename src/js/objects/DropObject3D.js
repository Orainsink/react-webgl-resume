...
class Drop {
	constructor(options) {
		this.parameters = Object.assign(Drop.defaultOptions, options);
	 ...
	 */
	getPlane() {
		let texture = new THREE.TextureLoader().load(require("../../assets/images/texture-drop.png"));
	...
	getTween(plane, index) {
		...
		const scale = ((index + 1) * this.parameters.amplitude) / this.parameters.count;
