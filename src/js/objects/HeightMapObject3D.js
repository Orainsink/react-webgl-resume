...
class HeightMap {
	...
	getLines() {
		...
		if (this.parameters.vertical) {
			for (let x = 0; x < this.parameters.divisionsX + 1; x++) {
				...
				for (let y = 0; y < this.parameters.divisionsY + 1; y++) {
					let vertex = this.geometry.vertices[x + (y * this.parameters.divisionsX + y)];
		...
		if (this.parameters.horizontal) {
			for (let y = 0; y < this.parameters.divisionsY + 1; y++) {
				...
				for (let x = 0; x < this.parameters.divisionsX + 1; x++) {
					let vertex = this.geometry.vertices[y * (this.parameters.divisionsX + 1) + x];
					lineGeometry.vertices.push(vertex);
	 ...
	 */
	loadMaps() {
		let totalData = (this.parameters.divisionsX + 1) * (this.parameters.divisionsY + 1);
	...
	applyMap() {
		let previousName =
			typeof this.previous === "undefined" ? "default" : this.parameters.maps[this.previous].name;
		...
		TweenLite.to({ factor: 1 }, 0.8, {
			...
			onUpdate: function() {
				for (let i = 0, j = _this.geometry.vertices.length; i < j; i++) {
					...
					let offset = currentData[i] + (previousData[i] - currentData[i]) * this.target.factor;
					vertex.z = offset;
				}
