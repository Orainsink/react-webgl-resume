...
import textureNeonProjectionImg from "../../assets/images/texture-neonProjection.png"
...
class Neon {
  constructor(options) {
    this.parameters = Object.assign(Neon.defaultOptions, options);
    ...
    this.idleIntensityTween = gsap.to(
      { projection: 0.08, glow: 0.4 },
      {
        ...
        onUpdate: function() {
          ...
          _this.glow.material.opacity = this._targets[0].glow;
          if (_this.parameters.projection) {
            _this.projection.opacity = this._targets[0].opacity;
          }
        },
        onComplete: yoyo,
    ...
    this.idleFlickTween = gsap.to({},{
      duration:  random(0.1, 10),
      paused: true,
      onComplete: function() {
        _this.flickOff();
  ...
  flickOn() {
    ...
    gsap.delayedCall(random(0.05, 0.07), () => {
